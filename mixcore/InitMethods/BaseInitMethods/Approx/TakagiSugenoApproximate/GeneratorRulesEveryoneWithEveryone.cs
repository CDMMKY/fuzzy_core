...
namespace FuzzySystem.TakagiSugenoApproximate.AddGenerators
{
    public class GeneratorRulesEveryoneWithEveryone : AbstractNotSafeGenerator
    {
        ...
        public static void InitRulesEveryoneWithEveryone(TSAFuzzySystem Approximate, TypeTermFuncEnum typeFunc, int[] countSliceForVar)
        {
            // i- номер строки 

            // j -номер входного параметра

            //Входное данное i, j    Approximate.LearnSamplesSet.DataRows[i].InputAttributeValue[j]
            // Выходное данное в строке i       Approximate.LearnSamplesSet.DataRows[i].Approx_Value



            if ((Approximate.RulesDatabaseSet == null) || (Approximate.RulesDatabaseSet.Count == 0))
            {
                KnowlegeBaseTSARules temp_rules = new KnowlegeBaseTSARules();
            ...
            while (counter[first_notNull] >= 0 && counter[0] >= -1)
            {
                ...
                double[] depencyCoefficent = null;
                double approx_Values = LSMWeghtReqursiveSimple.EvaluteConsiquent(Approximate, temp_term_set, out depencyCoefficent);
              
                TSARule temp_rule = new TSARule(Approximate.RulesDatabaseSet[0].TermsSet, order, approx_Values, depencyCoefficent);
