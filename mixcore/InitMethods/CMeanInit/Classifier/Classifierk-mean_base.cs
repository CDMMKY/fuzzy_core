...
{
    class Classifierk_mean_base
    {
        protected SampleSet learn_table;
        public SampleSet Learn_table { get { return learn_table; } }
        ...
        {
            int count_samples = learn_table.CountSamples;



            int mod;
            int count_point_to_cluster = Math.DivRem(learn_table.CountSamples, Count_clusters, out mod);
        ...
        protected void init_Distance_Matrix_d()
        {
            ...
            for (int i = 0; i < count_clusters; i++)
            {
                ...
                for (int j = 0; j < learn_table.CountSamples; j++)
                {
        ...
        protected virtual void Centroid_cordinate_S_Calc()
        {
            ...
            for (int i = 0; i < count_clusters; i++)
            {
                for (int j = 0; j < learn_table.CountVars; j++)
                {
                    ...
                    {
                        nominator += Math.Pow(u_Matrix[i][e], nebulisation_factor) * learn_table.DataRows[e].InputAttributeValue[j];
        ...
        protected virtual void calc_Distance()
        {
            ...
            for (int i = 0; i < count_clusters; i++)
            {
                for (int e = 0; e < learn_table.CountSamples; e++)
                {
                    ...
                    for (int j = 0; j < learn_table.CountVars; j++)
                    {
                        result += Math.Pow(Centroid_cordinate_s[i][j] - learn_table.DataRows[e].InputAttributeValue[j], 2);
                    }
        ...
        public Classifierk_mean_base(SampleSet Learn_table, int Max_iter, double precision_needed, int needed_count_clusters, double nebula)
        {
