...
namespace FuzzySystem.SingletoneApproximate.LearnAlgorithm
{
    public class OptimizeTermShrinkHardCode : AbstractNotSafeLearnAlgorithm
    {
        ...
        private KnowlegeBaseSARules MakeCut(KnowlegeBaseSARules Source, List<int> VectorCut, int NumVars)
        {

            KnowlegeBaseSARules result = new KnowlegeBaseSARules(Source);
            List<Term> cuttedTerms = result.TermsSet.FindAll(x => x.NumVar == NumVars);
            List<SARule> toChangeArules;
        ...
        private List<SARule> RemoveDuplicate(SAFuzzySystem Approximate ,List<SARule> Source)
        {
            List<SARule> result = Source;
            for (int i = Source.Count - 1; i > 0; i--)
            {
                for (int j = i - 1; j >= 0; j--)
                {
                    ...
                    {
                        Source.RemoveAt(i);
                        Source[j].IndependentConstantConsequent = KNNConsequent.NearestApprox( Approximate,Source[j].ListTermsInRule);
                        break;
                    }
                }
            ...
            public bool Equals(SARule x, SARule y)
            {
