...
namespace FuzzySystem.PittsburghClassifier.LearnAlgorithm
{
    public class MultiGoalOpimize : AbstractNotSafeLearnAlgorithm
    {
                ...
                for (int tr = 0; tr < trysBeforeDivide; tr++)
                {
                   ...
                   usedAlg = 0;
                    for(int i =0; i<learnAlgorithms.Count();i++)
                     {
                         Console.WriteLine("Fucked in Storage.Add(new c_Rules(Best))");
        ...
        private List<AbstractNotSafeLearnAlgorithm> initAlgoritms()
        {
            List<AbstractNotSafeLearnAlgorithm> result = new List<AbstractNotSafeLearnAlgorithm>();
            if (isTermShrink)
            {
            ...
            if (isBEE)
            {
                for (int i = 0; i < countBEE; i++)
                {
                    ...
                    result.Add(new FuzzySystem.SingletoneApproximate.LearnAlgorithm. Bee.BeeStructureAlgorithm());
                }
             }
            if (isES)
            ...
            if (isGA)
                ...
                for (int i = 0; i < countGA; i++)
                {
                    ...
                    result.Add(new GeneticAlgorithmTune.GeneticClassifier());
                }
          ...
          /*  if (isBFO)
                ...
                for (int i = 0; i < countBFO; i++)
                {
                    ...
                    result.Add(new BacterialForagingOptimization.Classifier.BacteryAlgorithm());
                }*/
        ...
        private int removeDublicate(List<double> ValueLGoods, List<double> ValueComplexityGoods, List<double> ValueInterapibility, List<double> ValueTGoods, List<KnowlegeBasePCRules> Storage, Random rand)
        {
            int result = 0;
           int indexforDel = 0;
        ...
        private double getComplexity(PCFuzzySystem Classifier)

        {
            switch (typeComplexity)
            {
                case 0: return Classifier.getComplexit();
                case 1: return Classifier.getRulesCount();
                default: return Classifier.getComplexit();
            }
        }
