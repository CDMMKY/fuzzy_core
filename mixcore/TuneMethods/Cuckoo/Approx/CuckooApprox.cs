...
{
    public class Cuckoo : AbstractNotSafeLearnAlgorithm
    {        
        double m;
        double p;
        int count_particle;
        ...
        public override SAFuzzySystem TuneUpFuzzySystem(SAFuzzySystem Approx, ILearnAlgorithmConf conf) // Здесь ведется оптимизация вашим алгоритмом
        {
            ...
            KnowlegeBaseSARules[] X = new KnowlegeBaseSARules[count_particle+1];
            ...
            {
                    X[0] = new KnowlegeBaseSARules(X[0]);                 
                 ...
                 for (int v = 0; v < m; v++)
                    {
                        ...
                        if (h > p)
                        {
                            X[ind] = new KnowlegeBaseSARules(X[ind]);  
