...
{
    public class Hooke_Jeeves_classifier : AbstractNotSafeLearnAlgorithm
    {
        int count_populate;
        int count_iteration;
        int iter;
        ...
        public override PCFuzzySystem TuneUpFuzzySystem(PCFuzzySystem Classifier, ILearnAlgorithmConf conf)
        {
            ...
            for (i = 0; i < Classifier.RulesDatabaseSet[0].TermsSet.Count; i++)
            {
                if (Classifier.RulesDatabaseSet[0].TermsSet[i].TermFuncType == TypeTermFuncEnum.Гауссоида)
                {
                    if (Classifier.RulesDatabaseSet[0].TermsSet[i].Parametrs[1] < 0.0000001)
                    {
                        Classifier.RulesDatabaseSet[0].TermsSet[i].Parametrs[1] = 1;

                    }
                }
            }

            MSE_best = Classifier.ClassifyLearnSamples(Classifier.RulesDatabaseSet[ 0]);
                ...
                if (iter == 0)
                {
                    ...
                    for (int h = 0; h < count_terms; h++)
                        for (int p = 0; p < type; p++)
                        {
                            ...
                            imput_var = Classifier.RulesDatabaseSet[0].TermsSet[h].NumVar;
                    ...
                    if (var[i] != 0)
                    {
                        ...
                        generate:
                            ...
                            if (type == 2)
                            {
                                ...
                                shrapnel = Proverka(shrapnel, Classifier, i, j, var);
            ...
            poisk_basis:
                if (st > 7) goto stop;
                point2 = Poisk_basis(point1, h1, count_terms, type, Classifier, MSE_1, flag, e, var);
            ...
            poisk_obraz:
                ...
                flag = 1;
                flag2 = 0;
                point4 = Poisk_basis(point3, h1, count_terms, type, Classifier, MSE_1, flag, e, var);
            ...
            stop:
                ...
                DB(point2, count_terms, type, Classifier);
                RMSE_pred[0] = RMSE_pred[1];
                RMSE_pred[1] = Classifier.ClassifyLearnSamples(Classifier.RulesDatabaseSet[ 0]);
        ...
        public double[] Poisk_basis(double[] basis, double[] h, int count_terms, int type, PCFuzzySystem Classifier, double MSE_1, int flag2, double e, double[] var)
        {
            int i, n;
            int flag = 0;
            n = basis.Length;
        ...
        poisk_basis:
            for (i = 1; i < n; i++)
            {
                if (var[i] != 0)
                {
                    ...
                    MSE_2 = DB(shrapnel, count_terms, type, Classifier);
                    if (MSE_2 <= MSE_1)
                    {
                        shrapnel[i] = basis[i] - h[i - 1];
                        MSE_2 = DB(shrapnel, count_terms, type, Classifier);
                        ...
                        {
                            shrapnel[i] = basis[i];
                            DB(basis, count_terms, type, Classifier);
                        }
                        else
                        {
                            MSE_1 = MSE_2;
        ...
        public double[,] Proverka(double[,] shrapnel, PCFuzzySystem Classifier, int n, int m, double[] var)
        {
            int type = Classifier.RulesDatabaseSet[0].TermsSet[0].CountParams;
            int variables = Classifier.LearnSamplesSet.CountVars;
            int count_terms = Classifier.RulesDatabaseSet[0].TermsSet.Count;
            shrapnel[0, 0] = 0;
            for (int i = 1; i <= n; i++)
            {
                if (var[i] != 0)
