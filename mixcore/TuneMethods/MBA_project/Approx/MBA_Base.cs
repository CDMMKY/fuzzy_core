...
namespace MBA_project
{
    public class Class1 : AbstractNotSafeLearnAlgorithm
    {
        ...
        public override SAFuzzySystem TuneUpFuzzySystem(SAFuzzySystem Approx, ILearnAlgorithmConf conf)
        {
            ...
            {
                if (Approx.RulesDatabaseSet[0].TermsSet[h].NumberOfInputVar == var) terms[var]++;
                else
                {
                    terms[var + 1]++;
                    var++;
            ...
            for (iter = 0; iter <= count_iteration; iter++)
            {
                ...
                for (i = 1; i <= Nd; i++)
                {
                    if (exploration > iter)
                    {
                        ...
                        generate:
                            ...
                            {
                                if (shrapnel[j, i] == Approx.LearnSamplesSet.InputAttribute(Approx.RulesDatabaseSet[0].TermsSet[(int)(i / type)].NumberOfInputVar).Min) i--;
                                goto generate;
                            }
                ...
                for (int h = 0; h < count_terms; h++)
                    for (int p = 0; p < type; p++)
                    {
                        ...
                        if (exploration > iter) d[0, k] = RandomNext(Approx.LearnSamplesSet.InputAttribute(Approx.RulesDatabaseSet[0].TermsSet[h].NumberOfInputVar).Min, Approx.LearnSamplesSet.InputAttribute(Approx.RulesDatabaseSet[0].TermsSet[h].NumberOfInputVar).Max);
