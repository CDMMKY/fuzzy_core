...
{
    public class Approx_discret : AbstractNotSafeLearnAlgorithm
    {
        int count_populate;
        int count_iteration;
        int exploration;
        ...
        public override TSAFuzzySystem TuneUpFuzzySystem(TSAFuzzySystem Approx, ILearnAlgorithmConf conf)
        {
            ...
            for (iter2 = 0; iter2 < iter_descrete; iter2++)
            {
                ...
                if (iter == 0)
                {
                    ...
                    {
                        d3[0, k] = RandomNext(Approx.LearnSamplesSet.InputAttribute(k).Min, Approx.LearnSamplesSet.InputAttribute(k).Max);
                    }
                }
                for (i = 0; i < variables; i++)
                {
                ...
                for (j = 1; j < count_populate; j++)
                {
                    for (int h = 0; h < variables; h++)
                    {
                        ...
                        else Approx.AcceptedFeatures[h] = false;
                    }
                    RMSE2[j] = Approx.approxLearnSamples(0);
