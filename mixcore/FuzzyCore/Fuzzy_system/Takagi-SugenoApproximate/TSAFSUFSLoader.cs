...
namespace FuzzySystem.TakagiSugenoApproximate.UFS
{
    public static class TSAFSUFSLoader : Object
    {
        ...
        public static TSAFuzzySystem loadUFS(this TSAFuzzySystem Approx, XmlDocument Source)
        {
            TSAFuzzySystem result = Approx;
            KnowlegeBaseTSARules New_dataBase = new KnowlegeBaseTSARules();
            ...
            for (int i = 0; i < count_rulles; i++)
            {
                ...
                double[] coefficients = new double[consequnt_node.ChildNodes.Count-1];
                for (int j = 0; j < consequnt_node.ChildNodes.Count; j++)
                {
                    string temp = consequnt_node.ChildNodes[j].Attributes.GetNamedItem("Variable").Value;
                    if (temp.Equals("IndependentValue", StringComparison.OrdinalIgnoreCase))
                    {
                        DoubleOutput = XmlConvert.ToDouble(consequnt_node.ChildNodes[j].Attributes.GetNamedItem("Value").Value);

                    }
                    else
                    {
                        int num_var = Approx.LearnSamplesSet.InputAttributes.IndexOf(Approx.LearnSamplesSet.InputAttributes.Find(x => x.Name.Equals(temp, StringComparison.OrdinalIgnoreCase)));
                        coefficients[num_var] = XmlConvert.ToDouble(consequnt_node.ChildNodes[j].Attributes.GetNamedItem("Coefficient").Value);
                    }
                }
                TSARule temp_rule = new TSARule(New_dataBase.TermsSet, Order_term, DoubleOutput, coefficients);
