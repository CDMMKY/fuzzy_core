...
{
    public static class LSMReqursive
    {
        ...
        public static double EvaluteConsiquent(TSAFuzzySystem System, List<Term> termSet, out double[] RegressionCoefficents)
        {
            ...
            int countPrams = termSet.Count();
            double a = Math.Pow(10, 100);
            Matrix P = new Matrix(countPrams + 1, countPrams + 1, 0);
            for (int i = 0; i < countPrams + 1; i++)
            {
                P.SetElement(i, i, a);
            }
            Matrix Teta = new Matrix(countPrams + 1, 1, 0);
            List<SampleSet.RowSample> RealValue = SelectPoint(System, termSet);
            List<SampleSet.RowSample> NominateValue = nominateValue(System, RealValue);

            for (int i = 0; i < NominateValue.Count; i++)
            {
                nominateVector = new Matrix(NominateValue[i].InputAttributeValue, countPrams + 1);
                UnidenominatorPart1 = nominateVector.Transpose().Multiply(P);
                Unidenominator2 = UnidenominatorPart1.Multiply(nominateVector);
                UniDenominator = 1.0 / (1.0 + Unidenominator2.GetElement(0, 0));
        ...
        public static List<SampleSet.RowSample> SelectPoint(TSAFuzzySystem System, List<Term> termSet, double SelectLevel = 0.5)
        {
      ...
      Math.Abs(System.LearnSamplesSet.DataRows[c].InputAttributeValue[termSet[i].NumVar] -
               termSet[i].Parametrs[0]);
                                ...
                                case TypeTermFuncEnum.Трапеция:
                                    double argvTR = (termSet[i].Parametrs[1] + termSet[i].Parametrs[2]) / 2;
                                    current_diff +=
                                   Math.Abs(System.LearnSamplesSet.DataRows[c].InputAttributeValue[termSet[i].NumVar] -
                                            argvTR);
                                    break;
                ...
                while (result.Count <= termSet.Count)
                {
